...
2. **HTML 解析**：若抓取结果是 HTML 源码，不要直接说“脚本不可用”，应调用 `run_skill_script_url-fetch`：
   - `script_path="extract_main_content.py"`
   - `cli_args_json=["--html","<抓取到的HTML>","--url","<可选URL>","--max_chars","8000"]`
   该脚本会输出 `MAIN_CONTENT`，可直接用于摘要或交给其他专家继续处理。

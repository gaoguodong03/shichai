...
- **工具名**：统一会话中为 `run_skill_script_<skill_id>`（本技能为 `run_skill_script_app-icon-generator`）。生成图标时只调用此工具，不要调用 list_allowed_directories、read_file 等文件/目录类 MCP 来「检查」配置。
- **命名约定（重要）**：必须优先输出 `run_skill_script_app-icon-generator`。若误输出 `run_skill_script`，后端会做别名映射作为兼容，但不建议依赖该兼容行为。

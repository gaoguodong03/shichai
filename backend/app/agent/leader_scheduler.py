...
- 若刚发言的专家尚未完成任务（需继续补充、调用工具、或回答不完整）→ task_done=false，next_speaker 通常为**另一位更合适的专家**或同一位专家的 agent_id（仅在确实需要该专家继续时）
- 若该专家已完成本轮任务 → task_done=true，next_speaker 为**下一位应发言的专家的 agent_id**（例如根据既定流程或你认为合适的人选）
- **当前成员无法完成工作时**：若缺少某类专家（如需要配图、核查、爬取等而群内没有对应专家），可输出 suggested_add_agent_ids（从「可邀请的新成员」中选 agent_id），并设 next_speaker="user"，让用户邀请新成员后再继续。
- **完成收敛规则（很重要）：**
...
- 主持人只负责调度（决定 task_done / next_speaker / suggested_add_agent_ids），不负责给专家写补充指令。
- 当建议邀请新成员时，输出 suggested_add_agent_ids（agent_id 数组）并 next_speaker="user"。

格式示例：
{{"task_done": true, "next_speaker": "agent_id或user或end", "reason": "简短理由", "suggested_add_agent_ids": ["agent_id1"]（可选，仅当需要邀请新成员时）}}

next_speaker 必须是当前参与者列表中的 agent_id 之一，或 "user" 或 "end"。"""


def _single_agent_extra_instruction() -> str:
    """仅一位专家时的额外说明：主持人可持续指定同一专家，直到出结果再交还用户。"""
    return """
**【当前仅有一位专家】** 你可多次指定同一专家（next_speaker 仍为该 agent_id）继续完成任务；直到该专家产出最终结果再设 task_done=true 且 next_speaker="user"。勿在未出结果时过早交还用户。"""
...
def _build_leader_prompt_with_single_hint(
    ...
    base = _build_leader_prompt(agent_list, discussion_goal, recent_messages, available_to_add)
    if len(agent_list) == 1:
        base += _single_agent_extra_instruction()
    return base
...
async def leader_decide(
    ...
    system_prompt = _build_leader_prompt_with_single_hint(
        agent_list, discussion_goal, recent_messages, available_to_add
    )
    ...
    try:
        ...
        if agent_list:
            ...
            fallback = last_speaker_agent_id or agent_list[0].get("agent_id", "user")
        else:
            fallback = "user"
        announcement = "请用户补充或继续提问。"
        if fallback and fallback != "user":
            for d in agent_list:
                if d.get("agent_id") == fallback:
        ...
        decision = OrchestrationDecision(
            ...
            owner_agent_id=fallback if fallback not in ("user", "end") else None,

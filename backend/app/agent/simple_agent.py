...
def _audio_transcription_final_message(tool_out: dict[str, Any]) -> AIMessage | None:
    """Return a deterministic full transcript message for audio transcription tools.

    Long transcripts should not be sent back through the LLM for restatement, because
    provider max output limits can truncate the user-visible answer. The script/MCP
    already returns the final transcript in JSON, so surface that text directly.
    """
    ...
    has_audio_asr_call = "audio-asr_transcribe_audio_file" in tool_names

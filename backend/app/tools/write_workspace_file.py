...
class WriteWorkspaceFileInput(BaseModel):
    ...
    path: str = Field(
        description=(
            ...
            "例如 notes/report-2026062519304500.md；只有用户明确指定已有路径或固定文件名时才按用户原文使用。"
        )
    )
    content: str = Field(
        default="",
...
def create_write_workspace_file_tool(workspace_id: str) -> ToolSpec:
    ...
    async def _write_to_workspace_file(path: str, content: str = "", overwrite: bool = False, **kwargs) -> str:
        ...
        if not path_value:
            return "错误：write_workspace_file 需要提供 path（workspace 内相对路径，例如 notes/report-2026062519304500.md）。"
    ...
    return ToolSpec.from_function(
        ...
        description=(
            ...
            "例如 'notes/report-2026062519304500.md'；用户明确指定已有路径或固定文件名时按用户原文使用。\n"
            "- content: 要保存的完整文本内容。"
        ),

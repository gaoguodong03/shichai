...
- 适用场景：一个专家绑定多个 Skill 目录时，后端按讨论目标与最近用户消息，用 `SkillsLoader` 基于名称/描述关键词等相关度选出 Skill；多 Skill 且未锁定时也可由专家模型择一，失败则回退上述逻辑。
- 路由入口：`app/skills/loader.py` 的 `pick_best_directory_name_for_message` / `pick_best_skill_with_debug`。
- Skill 会话锁：群聊中专家执行 Skill 后，后端会把 `skill_session_owner_name` 与 `skill_session_skill` 写入会话 meta；后续用户继续回复时，若未点名、未要求主持人接管、未覆盖 `next_speaker`，会跳过主持人并继续交给同一专家/Skill。

### Skill 会话退出协议

- 固定字段：脚本型 Skill 的 stdout JSON 必须输出 `execution_status`、`result_code`、`message`、`artifacts`、`next_action`。
- 会话字段：`next_action.skill_session` 只允许 `keep` 或 `release`。
- 字段含义：`release` 表示当前 Skill 流程已完成，应清除会话锁并交回主持人（四九）重新调度；`keep` 表示继续保持当前专家/Skill 锁，下一轮用户消息仍优先进入同一 Skill 会话。
- 专家正文状态块：普通专家正文如需控制 Skill 会话，也必须使用同一结构放入 `[[SKILL_SESSION_STATE]] ... [[/SKILL_SESSION_STATE]]`，状态块会被后端解析并从展示正文中移除。
...
- 字段主命名：`agent_names`、`leader_agent_name`
- 新接口只写出 name-based 字段；历史场景包和旧磁盘配置只在导入/读取边界做兼容转换。

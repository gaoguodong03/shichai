<template>
  <div class="rounded-xl border border-border bg-card shadow-sm">
    ...
    <div
      v-if="!editMode"
      class="skill-markdown-preview themed-scrollbar px-4 py-3 text-sm text-primary resize-y min-h-[14rem] overflow-auto break-words"
      v-html="previewHtml"
    />
    <textarea
      ref="textareaRef"
      v-else
      :value="body"
      class="w-full px-4 py-3 text-sm text-primary resize-y min-h-[14rem] themed-scrollbar border-0 bg-transparent rounded-none outline-none focus:outline-none"
      :disabled="!editMode"
...
function onInput(e: Event) {
  emit('update:body', (e.target as HTMLTextAreaElement).value)
  nextTick(() => autoResize())
}

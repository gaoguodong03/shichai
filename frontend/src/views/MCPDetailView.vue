<template>
  <div class="flex flex-col h-full bg-white overflow-y-auto">
    <header class="border-b border-gray-200 px-4 py-3 flex items-center justify-between">
      <h1 class="text-lg font-semibold text-gray-800">MCP Server 详情</h1>
    ...
    <form v-else-if="server" @submit.prevent="save" class="flex-1 overflow-y-auto p-4 space-y-4">
      ...
      <div class="flex items-center gap-2">
        <input v-model="form.enabled" type="checkbox" id="mcp-enabled" class="rounded border-gray-300" />
        <label for="mcp-enabled" class="text-sm text-gray-700">启用</label>
      </div>
      <div>
      ...
      <p class="text-xs text-gray-500">ID: {{ server.id }} · 状态: {{ server.status }} · 工具数: {{ server.tool_count ?? 0 }}</p>
    ...
    <div v-else class="p-4 text-gray-500">未找到该 Server</div>
  </div>
</template>

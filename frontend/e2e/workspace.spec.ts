...
test.describe('验收 2/6：工作空间会话与文件', () => {
  ...
  test('专家回复结束后刷新会话工作区文件与预览', async ({ page }) => {
    ...
    await page.route('**/api/sessions/session-existing/chat/stream', async (route) => {
      ...
      await route.fulfill({
        ...
        body: [
          `event: message\ndata: ${JSON.stringify({ message_id: 'assistant-file', speaker: { type: 'expert', agent_name: '问答专家' }, message: { content: '我已经更新工作区文件。' }, created_at: '2026053009000000' })}\n\n`,
  ...
  test('专家消息通过 message.artifacts 展示公开产物引用', async ({ page }) => {
    ...
    await expect(row.locator('.group-chat-execution-log-panel')).toContainText('执行工具：write_workspace_file')
  ...
  test('主持人消息也通过右侧终端入口展示调度日志', async ({ page }) => {
    ...
    await row.getByRole('button', { name: '查看工具日志' }).click()
    await expect(row.locator('.group-chat-execution-log-panel')).toContainText('执行工具：write_workspace_file')
    await expect(row.locator('.group-chat-execution-log-panel')).toContainText('选择发言人')
    ...
    await expect(detail.locator('div').first()).toContainText('时间')
    await expect(detail).toContainText('来源')
    await expect(detail).toContainText('结束原因')
  ...
  test('后端标记专家运行时显示占位气泡', async ({ page }) => {
    const state = createE2eState()
    state.sessions[0].agent_names = ['问答专家', '写作专家']
    state.sessions[0].runtime = {
      running: true,
      agent_name: '写作专家',
      skill: 'skill-write',
      phase: 'tool_running',
    ...
    await expect(placeholder.getByText('skill: 写作技能')).toBeVisible()
  ...
  test('后端明确建议下一位时显示确认提示', async ({ page }) => {
    await bootLoggedInApp(page)
    await page.route('**/api/sessions/session-existing/chat/stream', async (route) => {
      await route.fulfill({
        status: 200,
  ...
  test('只等待用户输入时保持普通发送态', async ({ page }) => {
    await bootLoggedInApp(page)
    const sentMessages: string[] = []
    await page.route('**/api/sessions/session-existing/chat/stream', async (route) => {
      const body = route.request().postDataJSON() as { message?: string }
      sentMessages.push(String(body.message || ''))
      await route.fulfill({
        status: 200,
        headers: { 'Content-Type': 'text/event-stream; charset=utf-8' },
        body: [
    ...
    await page.getByRole('heading', { name: '已有验收会话' }).click()
    await page.getByPlaceholder('输入消息').fill('请向我提一个问题')
    await page.getByRole('button', { name: '发送' }).click()

    await expect(page.getByText('已暂停：等待你的确认')).toHaveCount(0)
    await expect(page.getByText(/下一位：/)).toHaveCount(0)
    await expect(page.getByRole('button', { name: '确认并继续' })).toHaveCount(0)
    await page.getByPlaceholder('输入消息').fill('这是我的回答')
    await page.getByRole('button', { name: '发送' }).click()
    await expect.poll(() => sentMessages).toEqual(['请向我提一个问题', '这是我的回答'])
  })
  ...
  test('正在运行的会话在列表显示转圈，离开后完成显示新回复提示', async ({ page }) => {
    ...
    const streamStarted = new Promise<void>((resolve) => {
      page.route('**/api/sessions/session-existing/chat/stream', async (route) => {
        ...
        headers: { 'Content-Type': 'text/event-stream; charset=utf-8' },
        body: [
            `event: message\ndata: ${JSON.stringify({ message_id: 'assistant-late', speaker: { type: 'expert', agent_name: '问答专家' }, message: { content: '后台完成的回复' }, created_at: '2026052309300000' })}\n\n`,

...
test.describe('验收 5/6：设置中心', () => {
  test('用户可以配置主持人和配色', async ({ page }) => {
    ...
    await expect(page.locator('form > section').nth(0).getByText('项目整体系统提示词（所有角色共享）')).toBeVisible()
    ...
    await page.getByPlaceholder('写入适用于所有会话、场景、主持人和专家的项目规则，以及统一的工作区函数使用方式。').fill('自动化验收全局规则')
    await page.getByPlaceholder('定义主持人在所有场景中通用的纯调度职责、阶段表读取方式和输出结构。').fill('自动化验收主持人提示词')
  ...
  test('修改全局主持人名称后工作空间同步显示新名称', async ({ page }) => {
    ...
    await page.route('**/api/sessions/session-existing', async (route) => {
      ...
      const hostName = String(state.hostProfile.name || '四九')
      await route.fulfill({
        status: 200,

...
test.describe('验收 6/6：分享链接与公开场景路由', () => {
  test('用户打开 /share/run 分享链接后可以预览并确认导入', async ({ page }) => {
    ...
    await expect(page.getByText('场景名称：问答验收场景')).toBeVisible()
    await expect(page.getByText('包内专家')).toBeVisible()
    await expect(page.getByText('问答专家', { exact: true }).first()).toBeVisible()
    await expect(page.getByText('包内技能')).toBeVisible()
    await expect(page.getByText('问答技能')).toBeVisible()
    await expect(page.getByText('包内 MCP')).toBeVisible()
    await expect(page.getByText('mcp-files')).toBeVisible()
    await expect(page.getByText('"preset_id"')).toHaveCount(0)
    await expect(page.getByText('元信息摘要')).toHaveCount(0)

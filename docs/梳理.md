...
- **用户**：通过 `POST /api/group-sessions/{id}/chat/stream` 触发一轮会话。
- **主持人（Host）**：负责决定下一位发言专家、是否建议增援专家、是否暂停等待用户。
- **专家（DHA）**：绑定 `skill_ids`，执行具体任务，可调用工具（scripts / MCP / 文件系统等）。
- **SimpleAgent**：当前实际执行器（替代旧 LangGraph 主路径），识别工具意图并驱动工具调用。
...
- **会话存储**：
  ...
  - 专家记忆：工作区 `memory/messages`、`memory/logs`、`memory/facts.md`
...
1. 读取群聊历史、会话元信息、专家列表。
2. 解析请求参数（`message`、`override_next_speaker`、`host_takeover_requested`）。
3. 若开启 `FILE_REF_SERVER_RESOLVE_ENABLED`，对输入中的 `【文件引用：...】` 执行服务端展开（`resolve_file_refs_in_text()`）。

### 2.2 调度（主持人/回退）

在每轮执行前，系统会确定 `next_speaker`：

1. 优先使用显式 `override_next_speaker`（若合法）。
2. 否则走主持人决策 `_host_decide_by_dha(...)`。
3. 若主持人决策不可用，则回退 `leader_decide(...)`。
4. 决策结果经 `normalize_scheduler_decision(...)` 归一化，避免非法 speaker 或格式不一致。
...
2. 专家回合后写 memory：
   ...
   - `append_turn_log(...)`
   - `upsert_facts(...)`
3. 构造 `build_end_payload(...)`，写回 pending 状态与下一步建议，发送 `event: end`。
...
主流程在 `group_chat.py`，规则可概括为：

1. **人工优先**：用户显式指定下一位时，优先按 `override_next_speaker`。
2. **主持人优先**：调用 `_host_decide_by_dha()` 产出决策（含可能的建议增援）。
3. **鲁棒回退**：主持人不可用/返回不合法时，退回 `leader_decide()`。
4. **归一化**：`normalize_scheduler_decision()` 统一字段、校验候选人合法性。
5. **可中断**：遇到信息不足、需要用户确认、或招募专家时，转 `AWAITING_USER/RECRUITING` 并通过 `event: end` 传给前端。
...
- `append_turn_log(...)`：写 `memory/logs/*.md`，包含
  - `Tool Calls`
  - `Tool Raw Outputs`
  - `Tool Attempt Debug`
  - `Sandbox Entry Trace`
- `append_expert_message_file(...)`：写 `memory/messages/*.md`
- `upsert_facts(...)`：更新 `memory/facts.md`
- `build_dispatch_context(...)`：可构建派发上下文（供后续提示注入/检索）
...
1. **执行器历史切换**：主执行链已以 `SimpleAgent` 为准，仓库中仍可能有 LangGraph 历史代码片段（勿混淆主路径）。

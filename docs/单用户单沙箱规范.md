# 单用户单沙箱规范

本文档描述当前生效的沙箱模型。用于替代旧的按依赖哈希复用方案。

## 核心原则

- 一个用户只绑定一个沙箱实例（`1 user -> 1 sandbox`）。
- 不再计算或使用 `dep_hash`。
- 不再根据依赖哈希选择或复用基础镜像。

## 挂载与目录约定

- 用户 `workspaces` 根目录挂载到沙箱内 `/workspace`（可读写）。
- 当前 skill 的 `scripts` 目录挂载到 `/skill/scripts`（只读）。
- 可选的 skill `config` 目录挂载到 `/skill/config`（默认只读）。

## 会话隔离

- 同一用户多个会话共用同一个沙箱实例。
- 每个会话只允许访问自己的子目录：`/workspace/<session_id>`。
- 工具层（读写/重命名/创建目录/列表）必须做路径归一化和越界拦截。

## 脚本执行规则

- 运行入口固定为 `/skills/<skill_id>/scripts/<script_path>`；历史 `/skill/scripts/<script_path>` 路径仍做兼容回退。
- 脚本执行 `cwd` 固定为 `/workspace/<session_id>`。
- 脚本输入输出文件统一在 `/workspace/<session_id>` 下。
- 对历史技能提示中的 `scripts/config.json` 做兼容映射到会话根 `config.json`。
- 支持脚本后缀：`.py`、`.sh`、`.bash`、`.js`、`.mjs`、`.cjs`、`.ts`、`.tsx`；沙箱内分别使用 `python3/python`、`bash`、`node`、`tsx` 执行。

## 基础依赖

- 默认基础镜像复用应用镜像（1Panel 中 `SANDBOX_BASE_IMAGE` 默认跟随 `ST49_IMAGE`），主 `Dockerfile` 已内置常用 shell 命令、Python/Node 运行时、文档/图片处理工具与高频 Python 库。
- 如需独立维护 Skill 沙箱镜像，可使用 `docker/skill-sandbox/Dockerfile` 构建并通过 `SANDBOX_BASE_IMAGE` 覆盖。
- 修改镜像内容后需要重建并推送镜像，再重启后端/OpenSandbox 相关服务。
- 每用户额外 Python 包通过 `data/users/<username>/config/sandbox/requirements.txt` 管理；内容 hash 变化时会在该用户沙箱内重新安装。

## 审计口径

- `sandbox_session_created` 事件记录 `sandbox_mode=user_single_sandbox`。
- 不再输出 `dep_hash` 相关审计字段。

...
1. **技能选择与注入**：根据用户意图选定 Skill，将其完整指令（SKILL.md）注入到 Agent 的系统提示词；**只有这一个 Agent**，不再有子 Agent。
2. **按步骤执行**：Agent（LLM）按该 Skill 描述的步骤顺序执行；每一步可以是纯推理（如澄清需求、规划结构），也可以**在这一步里决定调用 MCP 工具**（如搜索、抓取网页）。
...
4. **无 Agent 调 Agent**：全程只有一个 ReAct 循环；Skill 只提供步骤说明，不发起新的 Agent 调用。

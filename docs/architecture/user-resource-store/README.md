...
本文档定义书童四九多用户资源存储的新标准。目标是把用户身份、资源中心、会话历史、平台内用户级环境变量和沙箱执行边界拆清楚，避免用户目录重建后资源丢失、沙箱误读完整用户数据、环境变量真实值被资源包带出等问题。
...
- `settings/` 存账号级应用设置、环境变量和沙箱依赖。
- 沙箱只拿执行视图，不直接获得完整用户身份、环境变量文件或账号密码。

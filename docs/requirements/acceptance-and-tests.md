...
书童四九是一个多用户隔离的 Agent 对话与工具平台，支持用户管理自己的专家（Agent）、场景、技能（Skills）、MCP 工具、LLM 配置与工作区文件，并通过统一会话完成主持人调度、专家协作、工具调用与结果沉淀。
...
| UR-03 主持人与专家协作 | 结构化入口路由、主持人决策、专家 runtime | `test_group_entry_router.py`、`test_sessions_api.py`、`test_host_takeover.py`、`test_expert_runtime.py` | 普通会话推荐专家、场景会话固定名单、`target_agent_name` 路由 | P0 必测 |
...
**需求**
- 用户可以新建、查看、更新、删除会话。
- 会话支持普通输入和从已导入场景新建协作会话。
- 会话流式返回专家输出、工具调用状态、主持人调度与最终结果。
- 会话历史与元信息可持久化并恢复。

**验收点**
- `GET /api/sessions` 返回当前用户会话列表。
- `POST /api/sessions` 可新建新会话，并生成唯一 `session_id`。
...
**需求**
- 用户可以新建、编辑、删除 Agent 专家实例。
- 专家可绑定 Skills、MCP 工具、头像与提示词配置。
- 专家配置可被会话调度与工具组装模块读取。

**验收点**
- Agent 列表只展示当前用户自己的专家。
...
| 核心编排层 | 结构化入口路由、主持人接管、专家 runtime 和场景会话恢复 | `pytest tests/test_group_entry_router.py tests/test_host_takeover.py tests/test_expert_runtime.py tests/test_sessions_api.py` |

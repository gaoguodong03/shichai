...
- 覆盖核心业务链路：编排、群聊、沙箱、鉴权、工作区、graph/工具/MCP 网关；
- 保持执行速度与稳定性；
...
- `test_graph_tool_resolution.py`：工具名解析、别名/非 ASCII 名称解析。
...
- `test_group_memory_store.py`：记忆日志、事实去重、分发上下文。
- `test_group_chat_group_memory.py`：群聊记忆注入、图片预览 markdown、自动继续信号。
...
- `test_public_scenario_api.py`：公开场景元数据与 bundle 下载。
...
适用账号：`2987371922@qq.com`

本次改动新增了“编写PPT”场景（引导专家 -> 图片专家 -> 引导专家组装 PPTX）的配置与技能脚本，建议按下面顺序做场景回归：

1. 在场景中输入少量想法（主题、受众、目标）；
2. 验证引导专家是否产出 `deck.json`，且包含 `deck_meta`、`style_guide`、`slides[]`；
3. 验证图片专家是否先出样张，再逐页生成图片并回写 `slides[].image_path`；
4. 验证引导专家是否调用 `generate_pptx.py` 输出 `final.pptx`。

### 已执行的本地校验

- JSON 结构校验：`session_presets.json`、`dha_instances.json` 均可正常反序列化；
- 脚本语法校验：`generate_pptx.py`、`generate_slide_image.py`、`pptx-deck-assembler/generate_pptx.py` 均通过 `py_compile`；
- 组装脚本试跑：在宿主环境执行时因缺少 `python-pptx` 失败（预期内），已在用户 `config/sandbox/requirements.txt` 增加 `python-pptx==1.0.2`，应在沙箱内满足依赖。

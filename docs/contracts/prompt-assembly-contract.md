...
`host.system_prompt` 是主持人长期提示词。前端创建主持人配置或新建场景时，把 `DEFAULT_HOST_SYSTEM_PROMPT` 直接填入对应输入框，点击保存后按表单原值写入后端。运行时只读取已保存值，不在字段缺失或空字符串时回退默认模板。它负责所有场景通用的纯调度职责、任务单要求、工作区调度边界、四列表读取方式和当前主持人 JSON 输出合同。

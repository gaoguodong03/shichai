...
6. **决定 `next_speaker`**  
   在已有专家的前提下，后端会结合：发言模式、调度器/主持人策略、用户是否手动指定下一轮发言人、是否有待恢复的专家等，算出 **下一个开口的 agent_id**（也可能是 `user`，表示轮到你输入）。  
   这里会出现「主持人先说一句再点名专家」的体验：例如先插入一条**主持人气泡**（「下面由某某发言」），**不单独结束流**，紧接着进入下面专家回合（同一条流里连续发生）。

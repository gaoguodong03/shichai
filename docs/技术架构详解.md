...
**技能**在本项目里主要是**用户目录下的一棵技能树**：每个技能一个子目录，**核心文件是 `SKILL.md`**（YAML frontmatter + Markdown 正文）。frontmatter 当前收敛为 **`name`**、**`description`**、**`allowed-tools`**：其中 `allowed-tools.mcp` 声明该技能运行时允许加载的 MCP，`allowed-tools.python` 仅作为依赖说明文本（不驱动服务端 pip 安装）。

- **存哪里**：`data/users/{用户名}/skills/{skill_id}/`  
  `skill_id` 一般与目录名一致；资源中心里创建/导入的技能会落在此路径。

- **加载与缓存**：[`loader.py`](../backend/app/skills/loader.py) 中的 **`SkillsLoader`** 扫描用户 `skills_dir`，解析 `SKILL.md`，在内存中维护 `skill_id → Skill`；启动时会预加载已存在用户的 Skills，运行期仍按用户隔离并在文件变更后失效缓存，避免多租户下共用一个全局单例竞态。群聊里若一位专家绑了多个 Skill，还会结合上下文做**选型**（`pick_best_skill_id` 等），决定本轮实际注入哪份正文。
...
1. **MCP 工具**  
   - 配置来自当前用户的 **`mcp_servers.json`**；启动时会预加载已存在用户的 MCP 配置并初始化非 lazy Server，运行时由 [`mcp/manager.py`](../backend/app/mcp/manager.py) 按用户维护 **`MCPToolManager`**（连接、按需加载 lazy Server、进程退出时清理）。  

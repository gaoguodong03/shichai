...
既然已经左侧栏目有了 file，能不能把 file的思路放进某个 chat 中，里面的dha 可以读取或者操作该 file：
也就是说新建某一个 chat 的时候为该 chat 分配一个工作区，里面有文件的的管理，众多的 dha 在文件中办公，dha 都可以读取其中的文件和修改其中的文件，也可以增加和删除其中的文件。
这里可能需要修改前端展示和存储结构，因为现在没有个 chat 中会包含一个 file 的结构，另外我觉得目前修改的一件事，对于目前最左侧的 Files 先不动，只是对于 chat 中的某个新建的对话加入file

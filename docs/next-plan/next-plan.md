...
- ~~已经建立的多DHA 对话中可以加入新的 dha~~ → 已实现：PUT add_dha_ids + 前端「邀请 DHA」按钮
- ~~对于 dha 来说，不应该再显示 MCP 的选择~~ → 已移除 DHA 的 MCP 勾选；skill 若用 MCP，DHA 自动可用全部 MCP
- ~~工具调用小蓝框，用户对话改为其他颜色~~ → 用户消息已改为 teal（青绿），工具小蓝框保留
- ~~"给下一个DHA 的提示词"改为具体的 dha~~ → 已改为「{{ next_dha_name }} 的提示词」“ 天气感知专家 的提示词”
- ~~发给 dha 的提示词以讨论目标为主~~ → 已改为「请紧扣讨论目标发言，不要偏离主题。」
---

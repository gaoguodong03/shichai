...
- `tool_start`、`tool_result` 不作为顶层 SSE 业务事件；工具细节写入执行 trace 或运行日志，用户可见产物写入 `message.artifacts`。
...
- 用户和主持人都通过标准消息的 `message.target_agent_name` 指定专家；内部可派生临时执行变量，`route_source` 仅用于后端日志和测试断言。
...
- 脚本 stdout 缺少完整 `expert_final_state.v2` 或字段非法：按该次脚本工具协议失败，保留日志；多个脚本结果可以不同，最终专家回复与控制状态以最终模型或 finalizer 输出为准。
...
- 跨用户路径：拒绝或返回不存在。
- 二进制预览不支持：提供下载或明确提示。
- finalizer 声称写入但产物引用与工具事实不一致：先允许一次真实产物工具调用纠正；仍不一致时移除无效引用、把该步记为失败并展示平台校验提示，不得发布原始保存声明或伪造产物路径。

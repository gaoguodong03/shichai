...
记忆功能负责管理 Agent 的短期和长期记忆，帮助 Agent 在对话中保持上下文和知识。记忆系统采用分层设计，包括 **轮对话（Session）** 级别的短期记忆和跨 Session 的长期记忆。

> **层级概念**：参见 [会话、轮对话与记忆设计](../architecture/session-round-memory.md)。  
> - **轮对话**：新建对话、历史会话都在这一层  
> - **轮内对话（Turn）**：每轮中的一次「用户消息 + 助手回复」  
> - **传给大模型的**：应是各 Turn 的**摘要或关键内容**，而非原始全文

## 功能特性

### 轮对话级别记忆（短期记忆）

轮对话（Session）级别的记忆用于管理**单轮**对话会话中的上下文信息：

- **轮内对话（Turn）管理**: 每轮中的每一次「用户消息 + Agent 回复」为一个 Turn
- **摘要优先**: 传给 LLM 的应为本轮各 Turn 的摘要或需要的内容，而非完整原始历史
- **Session 内的信息持久化**: 将完整对话内容保存，支持会话恢复和导出
- **上下文窗口管理**: 通过摘要控制送给 LLM 的 token 量，避免超出模型限制

...
专家最终输出的唯一结构是 `expert_final_state.v2`。平台从它生成 `ChatMessageRecord.message` 和 `skill_result`；工具结果、隐藏状态块和 deterministic summary 都不是消息来源。
...
| 字段 | 作用 |
| --- | --- |
| `schema_version` | 固定为 `expert_final_state.v2`。 |
...
| `next_action.agent_turn` | 当前专家本轮继续行动或回复用户。 |
| `next_action.skill_session` | 下一条用户消息是否回到同一专家和 Skill。 |

## 2. 两个控制维度

### `agent_turn`

- `continue`：当前专家仍需继续调用工具或处理；不落最终专家消息。
- `respond`：当前专家生成最终消息并结束当前回合。

工具循环由模型和运行时共同推进。MCP、HTTP、workspace 工具不输出 `next_action`；它们返回工具事实后，LLM 决定继续调用工具还是进入 finalizer。

### `skill_session`

- `keep`：下一条用户消息优先回到同一专家和同一 Skill。
- `release`：释放 Skill 会话，下一轮回到正常入口和主持人调度。

`agent_turn` 不决定下一条用户消息归属，`skill_session` 不决定当前工具循环是否继续。
...
禁止字段：顶层 `content`、顶层 `artifacts`、`handoff`、`resume`、`reason`、`instruction`、`result_code`、`workflow_state` 和隐藏状态块标记。

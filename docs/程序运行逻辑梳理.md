...
3. **生命周期**（`lifespan`）：启动时调用 `ensure_mcp_and_skills_initialized()`，扫描已存在的 `data/users/{用户名}`，逐个加载用户 Skills 与 MCP 配置，并初始化非 lazy 的 MCP Server；关闭时 `cleanup_all_mcp_runtimes()`，避免 asyncio cancel scope 跨任务问题。
